...
class Home_Model extends Model{
	...
	public function getNewProducts(){
		$sql = "SELECT * FROM products WHERE status=0 AND trash=0 ORDER BY created_at DESC LIMIT 9";
	...
	public function search()
		{
			$search = $_POST['search'];
			$sql = "SELECT * FROM products WHERE (product_name LIKE '%$search%' OR publisher LIKE '%$search%' OR author LIKE '%$search%')";
			$result = $this->getAll($sql);
			return $result;
		}
}
